...
IP_ADDRESS = "http://192.168.0.101:8090/"
...
class SoundInterface:
    ...
    def upVolume(self, val=1):
        newVol = "<volume>"+str(self.curVolume+(val))+"</volume>"
    ...
    def choosePreset(self,val):
        commandOne = requests.post(IP_ADDRESS+"key",KEY_PRESS+"PRESET_"+str(val)+KEY_END_TAG)
        commandTwo = requests.post(IP_ADDRESS+"key",KEY_RELEASE+"PRESET_"+str(val)+KEY_END_TAG)
    ...
    def togglePower(self):
        ...
        commandTwo = requests.post(IP_ADDRESS+'key',KEY_RELEASE+"POWER"+KEY_END_TAG)
        if self.power:
            self.power = False
        else:
            self.power = True
